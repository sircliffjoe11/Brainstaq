...
class DonorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create donor" do
    assert_difference('Donor.count') do
      post donors_url, params: { donor: { email: @donor.email, name: @donor.name, user_id: @donor.user_id } }
    end
  ...
  test "should update donor" do
    patch donor_url(@donor), params: { donor: { email: @donor.email, name: @donor.name, user_id: @donor.user_id } }
